...
1. 先ほどの[BRIDGE](https://thebridge.jp/)で検索結果を、ページをスクロールさせて取得させ、画像ファイルをダウンロードする

    - `Python`の`for`文で要素とインデックスを同時に取得する記述方法
    ...
    - `Python`で既存フォルダがあってもフォルダを新規作成する方法(`exist_ok=True`)
    ...
    次の通りフォルダに画像ファイルが保存されていれば成功！
    
    ![image](../imgs/bridge_search_images.png)